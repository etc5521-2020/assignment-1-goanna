...
The variables information included in the data are as the following table, latitude, longtitude and acq_date variables were mainly used for this analysis.    
...
In this analysis, date, temperature and temp_type variables for `temperature` data set were mainly used, and year, city_name and rainfall variables for `rainfall` data set as well.   
...
## When and where were the most widespread bushfires burning?  
...
### Which month did bushfires prefer? {#month}
...
rain_agv %>%
  ...
  ggplot(aes(x=month, y=`Average rainfall`) ) +
    ...
    xlab("Month")+
    ggtitle("Average monthly rainfall from 2001") 
...
### Which location did bushfires prefer?
...
## The higher the temperature, the more bushfires {#temp}
...
## The less rainfall, the more bushfire {#rain}

Has the less rainfall affected the more bushfire in Australia? Or has the more rainfall influenced the less bushfire?   Because of the location of Australia, the rainfall in Australia is highly variable, which is strongly influenced by global climate system phenomena such as El Niño, La Niña, and IOD. Despite this large natural variability, the potential long-term trends are evident in some regions, even effecting the local rainfall.   
In this section, we will explore the relationship between rainfall and bushfire, is it positive or negative?   
